...
public class MapGraph {
	...
	public List<GeographicPoint> dijkstra(GeographicPoint start, GeographicPoint goal, Consumer<GeographicPoint> nodeSearched)
	{
		...
		for (MapNode m : pointNodeMap.values()) {
            m.setDistance(Double.POSITIVE_INFINITY);
		}
		        ...
		        for (MapEdge edge : edges) {
					...
					if (!visitedNodes.contains(neighbor)) {
						...
						if (currentDist < neighbor.getDistance()) {
							...
							System.out.println("set distance: " + neighbor.getDistance() + " to neighbor: " + neighbor);
							queue.add(neighbor);
						} else {
							System.out.println("nope, neighbor.distance " + neighbor.getDistance() + " is longer than currentDist: " + currentDist);
						}
					} else {
	...
	{		
		...
		// expected: (1,1), (4,1), (5,1), (6.5,0), (8,-1)
		...
		System.out.print("DONE Loading the map");
		...
		List<GeographicPoint> routeDijkstra = theMap.dijkstra(start,end);
